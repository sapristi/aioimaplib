...
class IMAP4ClientProtocol(asyncio.Protocol):
    ...
    def _handle_responses(self, data: bytes, line_handler: Callable[[bytes, Command], Optional[Command]], current_cmd: Command = None) -> None:
        # Iterative: the previous tail-recursive version exceeded the
        # recursion limit on responses holding 1000+ lines in a single
        # chunk (see https://github.com/iroco-co/aioimaplib/issues/118).
        while True:
            if not data:
